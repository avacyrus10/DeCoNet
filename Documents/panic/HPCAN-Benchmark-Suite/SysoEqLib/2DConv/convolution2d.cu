...
__global__ void convolve2DKernel(const float* input, const float* kernel, float* output, int width, int height, int kernelSize) {
...
void convolve2D(const float* input, const float* kernel, float* output, int width, int height, int kernelSize) {
    ...
    convolve2DKernel<<<gridSize, blockSize>>>(d_input, d_kernel, d_output, width, height, kernelSize);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        std::cerr << "Failed to launch convolve2DKernel: " << cudaGetErrorString(err) << std::endl;
    }
