...
def load_2d_array(filename, shape):
    ...
    data = np.fromstring(' '.join(clean_lines), sep=' ', dtype=np.float32)
...
def validate_outputs():
    ...
    for filename in os.listdir(data_dir):
        if filename.startswith("input_"):
            ...
            subprocess.run(["./bin/correlate2D", input_file, kernel_file, output_file, str(width), str(height), str(kernel_size)])
