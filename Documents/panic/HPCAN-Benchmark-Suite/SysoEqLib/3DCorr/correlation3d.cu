...
__global__ void correlate3DKernel(const float* input, const float* kernel, float* output, int width, int height, int depth, int kernelSize) {
...
void correlate3D(const float* input, const float* kernel, float* output, int width, int height, int depth, int kernelSize) {
    ...
    correlate3DKernel<<<gridSize, blockSize>>>(d_input, d_kernel, d_output, width, height, depth, kernelSize);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        std::cerr << "Failed to launch correlate3DKernel: " << cudaGetErrorString(err) << std::endl;
    }
