...
from bs_mode_control import calculate_thinning_radius, apply_mode_control, compute_energy_per_info_bit
...
def main(K=None, visualize=True):
    # Generate network with optional K
    ...
    print(f"Merged major clusters: {len(merged_clusters)}")

    # Thinning + Mode Control
    ...
    eta_i = compute_energy_per_info_bit(num_awake, len(users_all))

    print("\n=== Base Station States ===")
    print(f"AWAKE: {num_awake} | SLEEP: {num_sleep} | ηᵢ: {eta_i:.4f} J/Mbit")
